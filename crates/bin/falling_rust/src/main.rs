...
use falling_rust::{WIDTH, HEIGHT, SCALE, Framework, Assets, UIInfo, NoiseRenderer, Color, Matrix};
...
fn main() -> Result<(), Error> {
    ...
    let mut matrix = Matrix::new_empty(WIDTH as usize, HEIGHT as usize, pixels.device());
    ...
    event_loop.run(move |event, _, control_flow| {
        ...
        if input.update(&event) {
            ...
            if input.key_pressed(VirtualKeyCode::C) {
                matrix = Matrix::new_empty(WIDTH as usize, HEIGHT as usize, pixels.device());
            };
            ...
            {
                matrix.update(&mut assets, pixels.device(), pixels.queue());
