...
use pixels::wgpu::util::{DeviceExt, BufferInitDescriptor};
use futures::executor::block_on;

use crate::{Assets, Material, Chunk, CHUNK_SIZE, WIDTH, HEIGHT, brush::Brush};
...
use shader_builder::compute::bind_groups::*;
use shader_builder::compute::*;
...
pub struct Matrix {
    ...
    compute_pipeline: pixels::wgpu::ComputePipeline,
}

impl Matrix {
    pub fn new_empty(width: usize, height: usize, device: &pixels::wgpu::Device) -> Self {
        ...
        Self {
            ...
            rng: rand::thread_rng(),
            compute_pipeline: shader_builder::setup_compute(device),
        }
    }
    ...
    // pub fn get_data_at_pos(&self, pos: IVec2) -> data_type {
    ...
    //             return None;
    //         };
    //         self.get_cell_from_cells(cell_idx as usize)
    //     }
    // }
    ...
    //             return None;
    //         };
    //         self.get_cell_from_cells_mut(cell_idx as usize)
    //     }
    // }
    ...
    //     cell.set_color(assets.get_color_for_material(cell.pos.into(), cell.material));

    //     let cell_at_pos = self.get_data_at_pos(cell.pos.into());
    ...
    pub async fn update(&mut self, assets: &mut Assets, device: &pixels::wgpu::Device, queue: &pixels::wgpu::Queue) {
        ...
        // let resulting_data = device.create_buffer(&pixels::wgpu::BufferDescriptor {
        //     label: None,
        //     size: self.data.len() as u64,
        //     usage: pixels::wgpu::BufferUsages::MAP_READ | pixels::wgpu::BufferUsages::COPY_DST,
        //     mapped_at_creation: false,
        // });

        // let mut encoder = device.create_command_encoder(&pixels::wgpu::CommandEncoderDescriptor { label: None });

        // {
        //     let mut cpass = encoder.begin_compute_pass(&pixels::wgpu::ComputePassDescriptor { label: None });
        //     cpass.set_pipeline(&self.compute_pipeline);
        //     self.compute_bind_group.set(&mut cpass);
        //     cpass.dispatch_workgroups(8, 1, 1);
        // }

        // // Sets adds copy operation to command encoder.
        // // Will copy data from storage buffer on GPU to staging buffer on CPU.
        // let src = &self.write_data_uniforms;
        // encoder.copy_buffer_to_buffer(src, 0, &resulting_data, 0, self.data.len() as u64);

        // // Submits command encoder for processing
        // queue.submit(Some(encoder.finish()));

        // // Note that we're not calling `.await` here.
        // let buffer_slice = resulting_data.slice(..);
        // // Sets the buffer up for mapping, sending over the result of the mapping back to us when it is finished.
        // let (sender, receiver) = futures_intrusive::channel::shared::oneshot_channel();
        // buffer_slice.map_async(pixels::wgpu::MapMode::Read, move |v| sender.send(v).unwrap());

        // // Poll the device in a blocking manner so that our future resolves.
        // // In an actual application, `device.poll(...)` should
        // // be called in an event loop or on another thread.
        // device.poll(pixels::wgpu::Maintain::Wait);
        ...
        //     drop(data);
        //     resulting_data.unmap(); // Unmaps buffer from memory
    ...
    pub fn draw(&self, screen: &mut [u8]) {
        ...
        for c in self.cells.iter() {
            ...
            let idx = (c.pos[0] as usize + c.pos[1] as usize * self.width) * 4;
            let pixel_color = &mut screen[idx..idx+4];
            let color = [(draw_color[0] * 255.0) as u8, (draw_color[1] * 255.0) as u8, (draw_color[2] * 255.0) as u8, (draw_color[3] * 255.0) as u8];
